...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(
            type="Pretrained",
            checkpoint="checkpoints/simmim_pretrain__swin_base__img192_window6__800ep.pth",
        ),
    ),
    decode_head=dict(
        type="IAParser",
    ...
    disentangle_head=dict(
        ...
        init_cfg=dict(
            type="Kaiming",
            override=dict(
                type="Constant", layer="Conv2d", name="reflectance_output", val=0.0, bias=0.5
            ),
        ),
...
log_config = dict(
    interval=50,
    hooks=[dict(type="TextLoggerHook", by_epoch=False), dict(type="TensorboardLoggerHook")],
)
...
gpu_ids = range(0, 1)
load_from = None
resume_from = "work_dirs/night_depth/latest.pth"
auto_resume = False
